...
MDIObject::MDIObject (QString profile, QString name)
{
  TA_RetCode rc = TA_Initialize();
  if (rc != TA_SUCCESS)
    qDebug() << "MDIObject::MDIObject: error on TA_Initialize";
...
int
MDIObject::message (ObjectCommand *oc)
{
  int rc = 0;
...
int
MDIObject::update (ObjectCommand *oc)
{
  clear();
  ...
  if (! io)
  {
    qDebug() << "MDIObject::update: invalid input" << _inputObject;
    return 0;
  }
  ...
  if (! io->message(&toc))
  {
    qDebug() << "MDIObject::update: message error" << io->plugin() << toc.command();
    return 0;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "MDIObject::update: TA-Lib error" << rc;
    return 0;
  }
...
int
MDIObject::dialog (ObjectCommand *oc)
{
  MDIDialog *dialog = new MDIDialog(oc->getObjects(), _name);
...
void
MDIObject::dialogDone (void *dialog)
{
  MDIDialog *d = (MDIDialog *) dialog;
...
int
MDIObject::outputKeys (ObjectCommand *oc)
{
  QStringList keys;
  keys << _outputKey;
...
int
MDIObject::load (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "MDIObject::load: invalid QSettings";
    return 0;
  }
...
int
MDIObject::save (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "MDIObject::save: invalid QSettings";
    return 0;
  }
