...
ADXRDialog::ADXRDialog (QHash<QString, void *> objects, QString name) : Dialog (0, name)
{
  QStringList tl;
  QDir dir(QDir::homePath());
  tl << dir.absolutePath() << QString("OTA") << QString("ADXR") << QString("settings") << QString("dialog");
...
ADXRDialog::~ADXRDialog ()
{
  saveSettings();
}

void
ADXRDialog::createTab (QHash<QString, void *> l)
{
...
void
ADXRDialog::setSettings (QString i, QString hk, QString lk, QString ck, int p)
{
  _input->setInput(i);
...
void
ADXRDialog::settings (QString &i, QString &hk, QString &lk, QString &ck, int &p)
{
  i = _input->input();
