...
ADXRObject::ADXRObject (QString profile, QString name)
{
  TA_RetCode rc = TA_Initialize();
  if (rc != TA_SUCCESS)
    qDebug() << "ADXRObject::ADXRObject: error on TA_Initialize";
  
  _profile = profile;
  _name = name;
  _plugin = QString("ADXR");
...
int
ADXRObject::message (ObjectCommand *oc)
{
  int rc = 0;
...
int
ADXRObject::update (ObjectCommand *oc)
{
  clear();
  ...
  if (! io)
  {
    qDebug() << "ADXRObject::update: invalid input" << _inputObject;
    return 0;
  }
  ...
  if (! io->message(&toc))
  {
    qDebug() << "ADXRObject::update: message error" << io->plugin() << toc.command();
    return 0;
  }
  ...
  if (! hbars)
  {
    qDebug() << "ADXRObject::update: invalid high bars" << _highKey;
    return 0;
  }
  ...
  if (! lbars)
  {
    qDebug() << "ADXRObject::update: invalid low bars" << _lowKey;
    return 0;
  }
  ...
  if (! cbars)
  {
    qDebug() << "ADXRObject::update: invalid close bars" << _closeKey;
    return 0;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "ADXRObject::update: TA-Lib error" << rc;
    return 0;
  }
...
int
ADXRObject::dialog (ObjectCommand *oc)
{
  ADXRDialog *dialog = new ADXRDialog(oc->getObjects(), _name);
...
void
ADXRObject::dialogDone (void *dialog)
{
  ADXRDialog *d = (ADXRDialog *) dialog;
...
int
ADXRObject::outputKeys (ObjectCommand *oc)
{
  QStringList keys;
  keys << _outputKey;
...
int
ADXRObject::output (ObjectCommand *oc)
{
  outputKeys(oc);
...
int
ADXRObject::load (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "ADXRObject::load: invalid QSettings";
    return 0;
  }
...
int
ADXRObject::save (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "ADXRObject::save: invalid QSettings";
    return 0;
  }
