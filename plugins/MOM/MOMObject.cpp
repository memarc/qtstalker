...
MOMObject::MOMObject (QString profile, QString name)
{
  TA_RetCode rc = TA_Initialize();
  if (rc != TA_SUCCESS)
    qDebug() << "MOMObject::MOMObject: error on TA_Initialize";
...
int
MOMObject::message (ObjectCommand *oc)
{
  int rc = 0;
...
int
MOMObject::update (ObjectCommand *oc)
{
  clear();
  ...
  if (! io)
  {
    qDebug() << "MOMObject::update: invalid input" << _inputObject;
    return 0;
  }
  ...
  if (! io->message(&toc))
  {
    qDebug() << "MOMObject::update: message error" << io->plugin() << toc.command();
    return 0;
  }
  ...
  if (! ibars)
  {
    qDebug() << "MOMObject::update: invalid input bars" << _inputKey;
    return 0;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "MOMObject::getMOM: TA-Lib error" << rc;
    return 0;
  }
...
int
MOMObject::dialog (ObjectCommand *oc)
{
  MOMDialog *dialog = new MOMDialog(oc->getObjects(), _name);
...
void
MOMObject::dialogDone (void *dialog)
{
  MOMDialog *d = (MOMDialog *) dialog;
...
int
MOMObject::outputKeys (ObjectCommand *oc)
{
  QStringList keys;
  keys << _outputKey;
...
int
MOMObject::output (ObjectCommand *oc)
{
  outputKeys(oc);
...
int
MOMObject::load (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "MOMObject::load: invalid QSettings";
    return 0;
  }
...
int
MOMObject::save (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "MOMObject::save: invalid QSettings";
    return 0;
  }
