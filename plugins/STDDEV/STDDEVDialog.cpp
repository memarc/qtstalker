...
STDDEVDialog::STDDEVDialog (QHash<QString, void *> objects, QString name) : Dialog (0, name)
{
  QStringList tl;
  QDir dir(QDir::homePath());
  tl << dir.absolutePath() << QString("OTA") << QString("STDDEV") << QString("settings") << QString("dialog");
...
STDDEVDialog::~STDDEVDialog ()
{
  saveSettings();
}

void
STDDEVDialog::createTab (QHash<QString, void *> l)
{
...
void
STDDEVDialog::setSettings (QString i, QString ik, int p, double d)
{
  _input->setInput(i);
...
void
STDDEVDialog::settings (QString &i, QString &ik, int &p, double &d)
{
  i = _input->input();
