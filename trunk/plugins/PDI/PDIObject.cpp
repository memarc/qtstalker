...
PDIObject::PDIObject (QString profile, QString name)
{
  TA_RetCode rc = TA_Initialize();
  if (rc != TA_SUCCESS)
    qDebug() << "PDIObject::PDIObject: error on TA_Initialize";
...
int
PDIObject::message (ObjectCommand *oc)
{
  int rc = 0;
...
int
PDIObject::update (ObjectCommand *oc)
{
  clear();
  ...
  if (! io)
  {
    qDebug() << "PDIObject::update: invalid input" << _inputObject;
    return 0;
  }
  ...
  if (! io->message(&toc))
  {
    qDebug() << "PDIObject::update: message error" << io->plugin() << toc.command();
    return 0;
  }
  ...
  if (! hbars)
  {
    qDebug() << "PDIObject::update: invalid high bars" << _highKey;
    return 0;
  }
  ...
  if (! lbars)
  {
    qDebug() << "PDIObject::update: invalid low bars" << _lowKey;
    return 0;
  }
  ...
  if (! cbars)
  {
    qDebug() << "PDIObject::update: invalid close bars" << _closeKey;
    return 0;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "PDIObject::update: TA-Lib error" << rc;
    return 0;
  }
...
int
PDIObject::dialog (ObjectCommand *oc)
{
  PDIDialog *dialog = new PDIDialog(oc->getObjects(), _name);
...
void
PDIObject::dialogDone (void *dialog)
{
  PDIDialog *d = (PDIDialog *) dialog;
...
int
PDIObject::outputKeys (ObjectCommand *oc)
{
  QStringList keys;
  keys << _outputKey;
...
int
PDIObject::output (ObjectCommand *oc)
{
  outputKeys(oc);
...
int
PDIObject::load (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "PDIObject::load: invalid QSettings";
    return 0;
  }
...
int
PDIObject::save (ObjectCommand *oc)
{
  QSettings *settings = (QSettings *) oc->getObject(QString("QSettings"));
  if (! settings)
  {
    qDebug() << "PDIObject::save: invalid QSettings";
    return 0;
  }
